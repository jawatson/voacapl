...
C. . .CALCULATE GAIN USING EQNS. (29) AND (34), PP. 11-12 
      W3 = COS (PSIV) 
      W4 = SIN (PSIV) 
      RAIN = 30.0 * ((A * (1.0 + CV * W3) + AS * CV * W4) * * 2 + (A * C
     1V * W4 + AS * (1.0 - CV * W3)) * * 2) / (RIN * T * * 2) 
...
C. . .CALCULATE THE GAIN. . . . . . . . . . . . . . . . . . . . . . . . 
      ZT = (SB * * 2 * Q * * 2 * AZV * CONJG (AZV) + CB * * 2 * AZH * CO
     1NJG (AZH)) * TT * * 2 
      SPSI2 = 1.0 - CPSI * * 2
      RAIN = 0.0
      IF (SPSI2 .EQ. 0.0) GO TO 615 
...
C.....ANTENNA ALREADY IN THE ARRAY, KOP=10..............................
 445  CONTINUE
      write (*,446)
 446  FORMAT (' Antenna type 10 not used for HFMUFES GAIN.')
      GO TO 640 
...
C.....ARBITARY TILTED DIPOLE ANTENNA, KOP=14............................
 ...
 580  continue
     ...
     1 1.0) * ((DIF - SQRD) / (DIF + SQRD)) * SR) * CMPLX (CR, - SR)) 
      RIN = R11 + CXC 
 585  RAIN = 120.0 * (ETHETA1 * * 2 + ETHETA2 * * 2 + EPHI1 * * 2 + EPHI
     12 * * 2) / RIN
      GO TO 615 
...
C.....ERROR EXIT........................................................
 635  write (*,636)
 636  format(' TIP > X in HUMUFES GAIN calculations.')
 640  STOP      
C 
C 
C 
