...
CRMFORT      IF (DHOPKM - DMAXKM (K))135, 230, 230
      IF (DHOPKM+0.1.GT.DMAXKM (K))GO TO 230
  135 CONTINUE
      IH = 0
      IL = 0
  140 IL = IL + 1
  145 IH = IH + 1
      IF(IH -44) 150,150,230
  150 IF (HPFLX (IH + 1, K))230, 230, 155
  155 CONTINUE
      IF (GDFLX (IH, K) - GDFLX (IH + 1, K))170, 160, 215
  160 IF (DHOPKM - GDFLX (IH, K))225, 165, 225
  165 DELMOD (IL, K) = DELFX (IH, K)
      ...
      GO TO 305
  170 IF(GDFLX(IH,K) - DHOPKM) 175, 165, 225
  175 IF(DHOPKM - GDFLX(IH+1,K)) 180, 176, 225
  176 IH = IH + 1
      GO TO 165
  180 CONTINUE
...
C.....BEGINNING OF SECTION TO SEARCH IN TABLE, DO LINEAR INTERPOLATION
      ...
      AFMOD (IL, K) = AFFLX (IH, K) + (AFFLX (IH + 1, K) - AFFLX (IH, K)
     1) * DTH / DTH2
      HPMOD (IL, K) = HP
      ST = SIN(THET)
...
C.....CHECK ON MINIMUM ANGLE AFTER INTERPOLATION
  305 IF(DELMOD(IL,K) - AMIND) 310, 315, 315
  310 HPMOD(IL,K) = - ABS(HPMOD(IL,K))
  315 CONTINUE
C.....END OF SECTION TO SEARCH IN TABLE, DO LINEAR INTERPOLATION
      IF (IL - 6)140, 230, 230
  215 IF(GDFLX(IH,K) - DHOPKM) 225, 165, 220
  220 IF(DHOPKM - GDFLX(IH+1,K)) 225, 176, 180
  225 GO TO 145
  230 CONTINUE
          NUMMOD=0
          do 236 ilm=1,6
